...
#' @param gradsArray Array of gradients of covariates, evaluated at
...
langevinUD <- function(locs, times, ID = NULL, grad_array, with_speed = TRUE){
  ...
  if (dim(grad_array)[1] != n)
    stop("The first dimension of gradientArray should be of size nrow(locs)")
  ...
  gamma_hat <- NULL
  if (with_speed){
    gamma_hat <- colSums( (Y -  sq_time_lag * grad_mat %*% nu_hat) ^ 2 ) / DF
    beta_hat <- nu_hat / gamma_hat * (DF - 2) / DF
    beta_hat_var <- (2 * beta_hat %*% t(beta_hat) / (DF - 4)
                        + nu_hat_var / gamma_hat * (1 + 2 / (DF - 4)))
  }
  else{
    beta_hat <- nu_hat
  ...
  conf_interval <- t(sapply(1:length(beta_hat), function(j){
    beta_hat[j] + c(1, -1) * qnorm(0.025) *  sqrt(beta_hat_var[j, j])
  }))
  rownames(beta_hat_var) <- colnames(beta_hat_var) <- paste0("beta", 1:J)
  rownames(conf_interval) <- rownames(beta_hat_var)
  return(list(betaHat = as.numeric(beta_hat), gammaHat  = gamma_hat,
