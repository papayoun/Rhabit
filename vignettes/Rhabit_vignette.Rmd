...
grad_list <- lapply(1:J, function(j){
                        tmp <- tracks[, (2 * (j + 1)):(2 * (j + 1) + 1)]
                        colnames(tmp) <- c("x", "y")
                        as.matrix(tmp)
                      })
names(grad_list) <- c('Var1', 'Var2')

grad_array <- do.call(function(...) abind(..., along = 3),grad_list)
```
...
formula_langevin_result <- fit_langevin_ud(cbind(x,y) ~ grad_c1, data = tracks)
```
...
```{r}
summary(langevin_result)
langevin_result$betaHatVariance
langevin_result$CI
```

**MPE essai avec la nouvelle ftonction**

```{r}
data_test <- cbind(locs)
head(data_test)
summary(langevin_result)
langevin_result$betaHatVariance
langevin_result$CI
