...
#' @param locs Point at which the gradient should be evaluated
...
gradLogUD <- function(beta, locs, xgrid, ygrid, cov_array) {
  grad_vals <- apply(cov_array, 3, function(cov_mat)
    nloptr::nl.grad(fn = interpCov, x0 = locs, xgrid = xgrid,
...
#' @export
covGradAtLocs <- function(locs, xgrid, ygrid, cov_array, lag_inter = 10) {
  x_size <- length(xgrid)
  y_size <- length(ygrid)
  grad_array <- do.call(function(...) abind::abind(..., along = 3),
                        lapply(1:dim(cov_array)[3], function(i){
                          t(apply(locs, 1, function(x){
                            ...
                            nloptr::nl.grad(fn = interpCov, x0 = x,
                                           xgrid = xgrid[x_inter],
                                           ygrid = ygrid[y_inter],
                                           cov_mat = cov_array[x_inter, y_inter, i])
                            }))
                        }))
  return(grad_array)
}
