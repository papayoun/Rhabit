...
cov_df <- do.call(rbind.data.frame,
                 lapply(1:J,
                        function(j){
                          rasterToGGplot(covariates[[j]], levels[j])
                        }))
...
The first step is to transform the list to an array:
...
set.seed(1) #repeatability
tracks <- simLangevinMM(beta = beta_true, gamma2 = 0.2,
                        times = times, loc0 = c(-5, 5),
                        xgrid = xgrid, ygrid = ygrid,
                        cov_array = cov_array, lag_inter = 3)
```
